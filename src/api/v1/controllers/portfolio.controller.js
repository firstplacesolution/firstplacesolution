...
} = require("../helpers/portfolio.helper");
    ...
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        if (money.base_wallet < amount) {
          ...
          let invested_amount =
            parseFloat(money.invested_amount) + parseInt(amount);
          let newBaseWalletBalance =
            parseInt(money.base_wallet) - parseInt(amount);
  ...
  getAll: async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        const data = await getPortfolioDetailsByInvestorId(
  ...
  removePortfolio: async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        ...
        const data = await portfolioModel.findOneAndRemove({
