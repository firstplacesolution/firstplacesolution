...
module.exports = {
    ...
    getStates: async (req, res) => {
        try {
            const stateDetails = await statesModel.find({}, { name: 1, id: 1 }).sort({'name' : 1});
    ...
    getCitiesByStateId: async (req, res) => {
        try {
            const citiesDetails = await citiesModel.find({state_id : req.params.state_id}, { name: 1 }).sort({'name' : 1});
